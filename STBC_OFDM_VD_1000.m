...
mse = [];
NofOFDMSymbol = 20;            %Number of data and pilot OFDM symbol
...
for snr = snr_min:step:snr_max; 
    ...
    for i=1:NofOFDMSymbol;
        ...
        for  k=1:length(s1_esti)
     
            for ll=1:length(y_16QAM)
                d(ll)=(HK(k)-1)*(abs(y_16QAM(ll))).^2+(abs(s1_esti(k)-y_16QAM(ll))).^2;
            end
            [D_min index]=min(d);
            s1_esti(k)=y_16QAM(index);
        end
        clear index;
        for  k=1:length(s2_esti)
    
            for ll=1:length(y_16QAM)
                d(ll)=(abs(HK(k))-1)*(abs(y_16QAM(ll))).^2+(abs(s2_esti(k)-y_16QAM(ll))).^2;
            end
            [D_min index]=min(d);
            s2_esti(k)=y_16QAM(index);
        end
        ...
        s1 = qamdemod(s1_est,M_ary);
        s2 = qamdemod(s2_est,M_ary);
...
save ser_vd_NoOFDMSymbol20_NT_100_itr_3_firsttest.mat psnr  SER;   
