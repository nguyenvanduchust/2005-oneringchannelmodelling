...
% Calculation of the time delay according to phi (Van Duc added on 24.08.05)

tau_in_samples = round(tau/ta); %?????????????????

%change the time delay in seconds to samples


N_P = length(channel_profile); % channel impulse response length
sum=zeros(1,N_P);
h=zeros(1,N_P);
count=zeros(1,N_P);

% Ring partition depending on tau
if i==1
    if j==1
        for k = 1:N_P
            for n=1:N
        ...
        for k = 1:N_P
            for n=1:N
                if (tau_in_samples(n)== (k-1))
                    ...
                    bn=exp(sqrt(-1)*pi*deltaMS*cos(phi2(n)-alphaMS));
                    fn=fD*cos(phi1(n)-alphav);
                    sum(k)=sum(k)+an*conj(bn)*exp(sqrt(-1)*(2*pi*fn*t+teta(n))); % ??????? why do they use "conj" function
                    count(k) = count(k) + 1;
                end
            end
            h(k)=channel_profile(k)*sum(k)/sqrt(count(k));
